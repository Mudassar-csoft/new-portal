...
{
    private const IMPORT_TAG = 'legacy_pending_web_leads_2026_06_29_v4';

    private const LEGACY_SOURCE_SITE = 'legacy.career.edu.pk';

    private const LEGACY_SOURCE_TABLE = 'web_leads';

    private const SOURCE_FILENAME = 'web_leads (3).sql';

    private const EXPECTED_PENDING_ROWS = 53;
    ...
    public function up(): void
    {
        ...
        if (count($pendingRows) !== self::EXPECTED_PENDING_ROWS) {
            throw new RuntimeException(sprintf(
                'Legacy web lead import aborted. Expected %d pending rows in %s, found %d.',
                self::EXPECTED_PENDING_ROWS,
                $sourcePath,
                count($pendingRows)
            ));
        }
        ...
        DB::transaction(function () use ($pendingRows, $campusLabels, &$existingLegacyIds, &$nextWebLeadId): void {
            $buffer = [];
    ...
    private function resolveSourcePath(): string
    {
        ...
        throw new RuntimeException(
            'Missing legacy web leads dump. Put `web_leads (3).sql` in storage/app/legacy-import, storage/app/private/legacy-import, legacy-import/, database/seeders/data/, or your Downloads folder.'
        );
    }
